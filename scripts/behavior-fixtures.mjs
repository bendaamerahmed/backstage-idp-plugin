  ...
  'dev-server': (root) => {
    ...
    write(
      ...
      [
        "import fs from 'node:fs';",
        '',
        '// The PID file is how the harness answers "was this left running?".',
        '// Process command lines do not carry a working directory, so scanning',
        '// for the fixture path misses exactly the orphan this scenario exists',
        '// to catch — the first version of this check did, and reported a clean',
        '// run while four dev servers were still alive.',
        "fs.writeFileSync(new URL('./.dev-server.pid', import.meta.url), String(process.pid));",
        '',
